"use strict";
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
Object.defineProperty(exports, "__esModule", { value: true });
var jsx_runtime_1 = require("react/jsx-runtime");
var utils_1 = require("../../utils");
var __1 = require("../..");
describe('<Indent />', function () {
    test('Should always render as is with default props', function () {
        var defaultProps = {};
        var wrapper = (0, __1.render)((0, jsx_runtime_1.jsx)(__1.Indent, __assign({}, defaultProps, { children: "Test" })));
        expect(wrapper).toEqual('Test');
    });
    test('Should always render indentation with spaces', function () {
        var defaultProps = {
            size: 4,
            type: utils_1.IndentationTypes.SPACES
        };
        var wrapper = (0, __1.render)((0, jsx_runtime_1.jsx)(__1.Indent, __assign({}, defaultProps, { children: "Test" })));
        expect(wrapper).toEqual('    Test');
    });
    test('Should always render indentation with tabs', function () {
        var defaultProps = {
            size: 2,
            type: utils_1.IndentationTypes.TABS
        };
        var wrapper = (0, __1.render)((0, jsx_runtime_1.jsx)(__1.Indent, __assign({}, defaultProps, { children: "Test" })));
        expect(wrapper).toEqual('		Test');
    });
    test('Should be able to make nest indentation', function () {
        var defaultProps = {
            size: 4,
            type: utils_1.IndentationTypes.SPACES
        };
        var wrapper = (0, __1.render)((0, jsx_runtime_1.jsx)(__1.Indent, __assign({}, defaultProps, { children: (0, jsx_runtime_1.jsx)(__1.Indent, __assign({}, defaultProps, { size: 2 }, { children: "Test" })) })));
        expect(wrapper).toEqual('      Test');
    });
    test('Should be able contain text component', function () {
        var defaultProps = {
            size: 4,
            type: utils_1.IndentationTypes.SPACES
        };
        var defaultTextProps = {
            indent: 4,
            type: utils_1.IndentationTypes.SPACES
        };
        var wrapper = (0, __1.render)((0, jsx_runtime_1.jsx)(__1.Indent, __assign({}, defaultProps, { children: (0, jsx_runtime_1.jsx)(__1.Text, __assign({}, defaultTextProps, { children: "Test" })) })));
        expect(wrapper).toEqual('        Test\n');
    });
});
//# sourceMappingURL=Indent.spec.js.map