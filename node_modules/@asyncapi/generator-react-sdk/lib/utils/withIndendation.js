...
function withIndendation(content, size, type) {
    ...
    if (content.includes('\n')) {
        ...
        return newLineArray.reduce(function (accumulator, value) {
            var newValue = value.trim() === '' ? value : "".concat(getIndentation(size, type)).concat(value);
            return accumulator === "" ? newValue : "".concat(accumulator, "\n").concat(newValue);
        }, "");
    }
    return "".concat(getIndentation(size, type)).concat(content);
}
